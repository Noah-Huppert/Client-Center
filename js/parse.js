	...
	$.get(xmlFile, function(data) {//Get XML File
		...
		$xml.find('outline').each(function() {
			if ($(this).attr('catagory') == 'true') {
				html.push('<details id="' + $(this).attr('text') + '" open><summary><a href="/' + subURL + "/" + $(this).attr('text') + '/">' + $(this).attr('text') + '</a></summary>');
...
function loading(progress){
	if(progress == 100){
		...
		setTimeout(function(){
			if($('#opmlListClone').children().children().length == 0){
				$('#glitch').show();
				setTimeout(function(){
					location.reload();
				}, 1500);
		...
		node.children().each(function() {
			if ($(this).children().length > 0) {//Has children
				if ($(this).parent().attr('catagory') == 'true') {//Post
					html.push('<details class="post" id="' + $(this).attr('text').split(' ').join('_') + '" data-created="' + $(this).attr('created') + '" data-link="/' + subURL + "/" + catagory + "/" + $(this).attr('text').split(' ').join('_') + '" open><summary>' + $(this).attr('text') + '</summary>');
					...
					if($(this).attr('closed') == 'true'){
						html.push('<details class="closed" close><summary>' + $(this).attr('text') + '<span class="mobileCollapse">Less</span><span class="mobileCollapse">Less</span></summary>');
						checkChildren($(this), displayElement, subURL);
						html.push('</details>');
					}
					if($(this).attr('image') != 'true' && $(this).attr('closed') != 'true'){
						html.push('<details open><summary>' + $(this).attr('text') + '</summary>');
						checkChildren($(this), displayElement, subURL);
						html.push('</details>');
					}
				}
				...
				if ($(this).parent().attr('catagory') == 'true') {//Post
					html.push('<details class="post" id="' + $(this).attr('text').split(' ').join('_') + '" data-created="' + $(this).attr('created') + '" data-link="/' + subURL + "/" + catagory + "/" + $(this).attr('text').split(' ').join('_') + '" open><summary>' + $(this).attr('text') + '</summary></details>');
				} else {//Not Post
					if($(this).attr('image') == 'true'){
						html.push('<img class="embededIMG thumbnail" src="' + $(this).attr('imageSRC') + '" />');
					}

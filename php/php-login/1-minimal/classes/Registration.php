...
class Registration
{
    ...
    private function registerNewUser()
    {
		...
		//Projects
		$sql = "INSERT INTO  `clientcenter`.`projects` (
			`client` ,
			`name`
			)
			VALUES (
			'" . $_POST['user_name'] . "',  '" . $_POST['project_name'] . "')";
		$query = mysqli_query($connect, $sql);
		
		$id;
		
		//Project ID
		$sql = "SELECT `id` FROM `projects` WHERE `client`='" . $_POST['user_name'] . "' ORDER BY  `id` DESC LIMIT 0 , 1";
		$query = mysqli_query($connect, $sql);
		...
		//Preview Address
		$sql = "INSERT INTO  `clientcenter`.`previewaddress` (
			`address` ,
			`projectID`
			)
			VALUES (
		...
		//Hours
		/*$sql = "INSERT INTO  `clientcenter`.`hours` (
			`hours` ,
			`projectID`
			`date`
			)
